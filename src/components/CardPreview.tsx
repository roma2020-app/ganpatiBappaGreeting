...
export const CardPreview: React.FC<CardPreviewProps> = ({ card, onReset, onUpdateCard }) => {
  ...
  // Format personalized greeting generated by the application
  const getPersonalizedGreeting = () => {
    const lines: string[] = [];
    if (card.title) lines.push(card.title);
    if (card.recipientSalutation) lines.push(card.recipientSalutation);
    if (card.message) lines.push(card.message);
    if (card.tagline) lines.push(card.tagline);
    if (card.signature) lines.push(card.signature);
    return lines.filter(Boolean).join('\n\n');
  };

  // WhatsApp Message matching user specification:
  // 🙏 गणपती बाप्पा मोरया! 🌺
  //
  // I created a special Ganpati greeting for you. ❤️
  //
  // [Insert the personalized greeting generated by the application]
  //
  // ✨ Create your own personalized Ganpati greeting:
  // https://ganpatigreeting.vercel.app/
  const getWhatsAppMessage = () => {
    const personalizedContent = getPersonalizedGreeting();
    return [
      '🙏 गणपती बाप्पा मोरया! 🌺',
      '',
      'I created a special Ganpati greeting for you. ❤️',
